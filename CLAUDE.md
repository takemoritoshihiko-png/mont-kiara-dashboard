...
## 🛑 サブエージェントのモデル選択（2026-08-09 竹森氏指示・恒久）

**調査・読取・検証系のエージェントは `model: haiku` が既定**（Web調査・数値再読取・座標確認・生存確認・チェーン店舗数カウント等）。分析・思考（RCA・矛盾裁定）のみ `opus`。提案・設計はサブに出さずメインが担う。

- **opusを使うときは「なぜhaikuでは足りないか」を発注報告に1行書く**。書けないならhaikuで出す
- 発注を報告するときは**各エージェントの使用モデルを明記**する（竹森氏が目視で監査できる出力契約）
- 迷ったら安い方→不十分なら上位で再実行
- 由来: 2026-08-09の実事故（KL飲食調査で十数体を全部opus発注）。全プロジェクト共通ルールはAIメモリ `feedback_subagent_model_selection` が正
