...
KLとペナンの**コンドミニアム271・学校33・商業施設33(KL有名モールTOP10+「200店以上×MK車1時間圏」16+MK付近50店以上6・旧88件はdocs/archiveに保全)・飲食店356(データ行374・墓標17・閉店1=Niko Neko Matcha)**を1枚の地図で見くらべる、日本人向けの住まい探しダッシュボード。
...
## ⏱ 検証にかける時間の作法（2026-08-16 実測にもとづく）

2026-08-16の作業2時間6分を計測した結果、**テストは全体の1.7%（27回で131秒）** しか使っていない。詰まるのは別の場所なので、次を守る。

- **テストは安いのでこまめに走らせる**。`npx` 経由は解決に0.9秒かかるので **`npm test`（1.99秒）か `./node_modules/.bin/vitest run`（1.65秒）** を使う
- **ブラウザ検証は最も高い**（181回で586秒＝ツール時間の40%）。そのうち **185秒は6回のタイムアウト**だった。避け方:
  - 見た目の確認は `screenshot` より **`javascript_tool` で数値を測る**（速い・切れているかも判定できる）
  - 地図の再描画・iframe読み込みの**直後に screenshot / zoom を撃たない**（レンダラが詰まって30秒タイムアウトする）
  - 独立した操作は1回のバッチにまとめる
- **CI待ちに `Start-Sleep` を使わない**（4回で229秒を寝ていた）。`gh run watch <id> --exit-status` か `run_in_background` を使う
- `git push` は42〜64秒かかる（実測）。これは回線で、削れない
