...
KLとペナンの**コンドミニアム271・学校33・商業施設10(KL有名モールTOP10・旧88件はdocs/archiveに保全)・飲食店357(データ行374・墓標17)**を1枚の地図で見くらべる、日本人向けの住まい探しダッシュボード。
...
## 🛑 サブエージェントのモデル選択（2026-08-09 竹森氏と確定・恒久）

**「判断の量」で4層に分類し、`model` を毎回明示指定する**:

| 層 | 使う場面 |
|---|---|
| `haiku` | 判断ゼロの読取・転記（閉店フラグ・数値読み・grep・座標機械確認） |
| `sonnet` | **軽い判断つき調査=調査の主力**（チェーン店舗数カウント・分類・命名・整形・複数ソース一次突合）。精度は指示の具体性で担保 |
| `opus` | 重い分析（RCA・多要因の矛盾裁定）+sonnet失敗時の再実行先 |
| `fable` | 例外3パターンのみ（コンテキスト逼迫時の設計代行/敵対的最終ジャッジ/設計案の並列生成）。通常の設計・提案はメインが担う |

- **失敗コスト補正**: 一括検証など失敗時の再発注が高くつくタスクは最初から1ランク上を使ってよい（判断を1行書く）
- **出力契約**: 発注報告に各体の使用モデルを明記＋**sonnet以上は「なぜ下位で足りないか」1行**。書けないなら1ランク下げる
- **安全例外**: 安全マターはコスト度外視でopus以上+メイン最終確認
- パイプライン推奨: 収集(haiku)→検証・整形(sonnet)→矛盾裁定のみ(opus)
- 由来: 2026-08-09実事故（全調査をopus発注）→同日4層版に改訂。全プロジェクト共通の正はAIメモリ `feedback_subagent_model_selection`
