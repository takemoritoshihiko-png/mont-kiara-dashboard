...
function mkMarker(c) {
  ...
  if (isCommercial) {
    ...
    // NLAが公表されていない施設(GM Klang Wholesale City=非REITの卸売モール)は
    // 面積が0扱いになり、最大級でも最小の22pxで描かれてしまう。面積が無いときは
    // **店舗数で段を選ぶ**（面積の代わりに規模を語れる唯一の実測値）。
    // 数字を22pxに詰め込むと4桁が枠から溢れる、という実害もここで消える。
    const csz = nla > 0
      ? (nla >= 1000000 ? 30 : nla >= 400000 ? 26 : 22)
      : (tenants >= 700 ? 30 : tenants >= 200 ? 26 : 22);
