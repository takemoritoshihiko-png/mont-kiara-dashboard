...
| ファイル | 役割 |
|---|---|
| `src/ui/map.js` | Leaflet地図・マーカー生成（種別別の見た目。**ミシュランは星=濃い金+★／掲載店=淡い金+Ⓜ／ビブ=琥珀+🍽 の3段。並びは絞り込みの選択肢と凡例で揃える**）・**外食モードのピンに ✓訪問済み(右上) と ♡行きたい(左上) のバッジ**・クラスタ・ラベルのズーム連動・エリアジャンプ（住まい層は `fArea`、飲食層は**中心からの半径3km**で絞る）・凡例 |
