...
| `src/domain/filter.js` | 層の判定（`recordLayer`）・全絞り込み条件の判定・カリキュラム一覧・帯レンジのパース・飲食の9分類/ミシュラン/価格帯（`diningPriceCeiling`＝昼夜どちらの上限で見るかを引数で受ける）・**飲食の距離フィルタ `matchesDiningNear`（半径 `NEAR_KM`=3km）** |
...
| ファイル | 役割 |
|---|---|
| `src/ui/map.js` | Leaflet地図・マーカー生成（種別別の見た目）・クラスタ・ラベルのズーム連動・エリアジャンプ（住まい層は `fArea`、飲食層は**中心からの半径3km**で絞る）・凡例 |
...
| `test/dining.test.js` | **飲食データの契約**: 93件(v9系50+拡充43)・id/placeId一意・座標域・価格 lo≤hi・9分類・ミシュランenum |
